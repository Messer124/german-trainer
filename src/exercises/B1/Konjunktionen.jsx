...
export default function Konjunktionen() {
    ...
    const sections = useMemo(
        () => normalizeExerciseSections(data, locale, "konjunktionen"),
        [locale]
    );
    ...
    const handleChange = (answerKey, value, acceptedAnswers) => {
        const key = `konjunktionen-${answerKey}`;
    ...
    const renderInput = (row, inputMode) => {
        const key = `konjunktionen-${row.answerKey}`;
        ...
        return (
            <ExpandingInput
                ...
                minWidth={isInlineBlank ? 95 : 340}
                mobileMinWidth={isInlineBlank ? 65 : 150}
                tabletMinWidth={isInlineBlank ? 80 : 240}
                maxWidth={isInlineBlank ? 220 : 900}
                aria-label={`Konjunktionen answer ${row.answerKey}`}
                enableHint
